...
typedef void (*ParseFn)(bool canAssign); // ParseFn = 指向某個parser函式的指標  ParseFn 是指標指向一個回傳 void 的函式
...
{ // 用addConstant()得到value存放的位置然後放進chunk裡面
    int constant = addConstant(currentChunk(), value);
...
static void varDeclaration() {
    ...
    consume(TOKEN_SEMICOLON, "Expect ';' after variable delcaration.");
